...
import { fetchAllValidSymbols } from '../actions/stock';

class NewBuyForm extends React.Component {
  constructor() {
    super();
    this.state = {
      symbol: '',
  ...
  handleSubmit(evt) {
    ...
    this.props.addStockTransaction(this.props.userId, symbol, shares);
  }
    ...
    return (
      <div className="trade-view">
        ...
        {error && error.response && (
          <div className="err-message"> {error.response.data} </div>
        )}
      </div>
    );
  }
...
const mapDispatch = dispatch => {
  return {
    ...
    addStockTransaction: (userId, symbol, shares) =>
      dispatch(addStockTransaction(userId, symbol, shares))
  };
};
